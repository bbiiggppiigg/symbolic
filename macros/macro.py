...
from macros.bounded_expression import OutputActionList, OutputAction
...
class PrecedenceFactory(object):
    ...
    def insert(self, before, after):
        ...
        if before_fvs.mapping == dict() or after_fvs.mapping == dict():
            # is non-symbolic:
    ...
    def record(self, pkt):
        ...
        for key, prec in self.symbolics.items():
            if prec.before_trivially_satisfied(pkt):
                self.record[key] += prec.get_filter(pkt)
    ...
    def create_output_action_list(self, pkt):
        ...
        for key, prec in self.concretes.items():
            ...
            # If there is no
            ret += prec.create_output_action_list()

        for key, prec in self.concretes.items():
            ret += prec.create_output_action_list(self.record[key])
...
class NewConcretePrecedence(Macro):
    ...
    def before_trivially_satisfied(self, pkt):
        ...
        if self.before_output is None:
            try:
                if self.before_input.instantiate_fvs(MapFVValue(pkt, None)).apply(pkt).is_true():
                    return True
    ...
    def after_trivially_unsat(self, pkt):
        if self.after_input is not None:
            try:
                if not self.after_input.instantiate_fvs(MapFVValue(pkt, None)).apply(pkt).is_true():
                    return True
            except UnsatisfiableActionException:
                return True
    ...
    def create_output_action_list(self, pkt):
        ...
        if self.before_output is not None:
            before_output = self.before_output.instantiate_fvs(MapFVValue(pkt, None)).apply(pkt)
            choose_before = before_output.associate([(0, self.id, 0)])
            ret.append(choose_before)

        if self.after_output is not None:
            after_output = self.after_output.instantiate_fvs(MapFVValue(pkt, None))
    ...
    def get_filter(self):
        ...
        return self.after.instantiate_fvs(MapFVValue(pkt, None)).apply(pkt).get_filter()
...
class NewSymbolicPrecedence(Macro):
    ...
    def before_trivially_satisfied(self, pkt):
        ...
        conf = MapFVValue(self.fvs, pkt)
        if self.before_output is None:
            if self.before_input.instantiate_fvs(MapFVValue(pkt, conf)).apply(pkt):
                return True
    ...
    def after_trivially_unsat(self, pkt):
        # type:(MapInputValue) -> bool
        conf = MapFVValue(self.fvs, pkt)
        if not self.after_input.instantiate_fvs(MapFVValue(pkt, conf)).apply(pkt):
            return True
        return False

    def create_output_action_list(self, pkt, filter):
        ret = []
        if self.before_output is not None:
            before_output = self.before_output.instantiate_fvs(MapFVValue(pkt, None)).apply(pkt)
        ...
        if self.after_output is not None:
            ...
            for i, bp in enumerate(bplist):
                if i != after_output.index:
                    ...
                    ret.append(OutputAction([], [(1, self.id, 1)], filter))
        else:
            if self.before is None:
                assert False, "We're doomed! "
